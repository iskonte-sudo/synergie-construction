...
- **Emergent Object Storage — 2026-02** ✅ Uploads migrated from ephemeral local disk to persistent Emergent Object Storage (`INTEGRATION_PROXY_URL` + `EMERGENT_LLM_KEY`). Files stored at `synergie/uploads/{folder}/{name}`. GET `/api/uploads/{path}` proxies from objstore with local disk fallback for legacy files. Fixes production upload bug (ephemeral pods).
- **Slider Admin UX fix — 2026-02** ✅ `GenericCrud.jsx` modal no longer auto-closes when the native file picker returns (safe outside-click uses mousedown+click on backdrop). Instant blob preview during upload, spinner overlay, X clear button, image `onError` handler, client-side validation (image type + 15MB max), verbose error toast with backend detail. Applies to slides, testimonials, team, partners. Validated by testing agent 100% (iteration_4.json).
